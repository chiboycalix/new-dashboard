    ...
    const fetchUsers = async () => {
      ...
      userRef.on("value", (snap) => {
        ...
        for (let m in userInstance?.PostLikes) {
          likes.push({
            id: m,
            length: Object.keys(userInstance?.PostLikes[m]).length,
          });
        }
        for (let m in userInstance?.["Post Comments"]) {
          comments.push({
            id: m,
            length: Object.keys(userInstance?.["Post Comments"]?.[m]).length,
          });
        }
        for (let m in userInstance?.["all_re_post"]) {
          regist.push({
            id: m,
            length: Object.keys(userInstance?.["all_re_post"]?.[m]).length,
          });
        }
        setUser({
          ...user,
  ...
  const sendPush = (email) => {
    ...
    const options = {
      ...
      headers: new Headers({
        ...
        Authorization:
          "key=AAAApdxljRk:APA91bH3HxOyY8B2yJjT9XG93RiyfR6mixuTB673tWoItmOzzs1WEfMSCUiSLxNJeREdxzIYjt82CXdUOaXPmmrIAGEwUeBB1xIoFPssWS6oiBnX9rKfoSkuj7-JgYYl7sI4TaqZKKRSZ4P02h41PO020bNhCwtlTw",
                ...
                <div className="col-md-6">
                  <div>
                    ...
                    <Tooltip title={user?.profile?.email}>
                      <p className="bold-content  truncate">
                        {user?.profile?.email}
                      </p>
                    </Tooltip>
                  </div>
            ...
            <div className="other-section">
              <div className="tab-header-container">
                ...
                <div
                  ...
                  className={`tab-header ${
                    activeTab === 2 ? "active-header" : ""
                  }`}
                >
                  <p>Peer</p>
                  <img src={DotsIconRed} alt="peer" />
                </div>
                <div
                  onClick={() => setActiveTab(3)}
                  className={`tab-header ${
                    activeTab === 3 ? "active-header" : ""
                  }`}
                >
                  <p>Group</p>
              ...
              <div className="tab-content-container">
                        ...
                        <div className="left-section">
                          ...
                          <div className="text-section">
                            ...
                            <p className="posted-date">
                              Posted on {new Date(value?.date).toDateString()}
                            </p>
                            <p className="gist-content">{value?.text}</p>

                            <div className="stats-section">
                              <div className="stat-content">
                                <img src={Like} alt="likes" />{" "}
                                <span>{postLikes(value.postid)}</span>
                              </div>
                              <div className="stat-content">
                                <img src={Comment} alt="comments" />{" "}
                                <span>{postComments(value.postid)}</span>
                              </div>
                              <div className="stat-content">
                                <img src={Regist} alt="regist" />{" "}
                                <span>{postRegist(value.postid)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="image-section">
                          {value.imgarray?.length && (
                            <img
                              src={value?.imgarray?.[0]?.imgurl}
                              alt="placeholder"
                            />
                          )}
                          {value.videourl?.length && (
                ...
                <div
                  className={`content ${
                    activeTab === 2 ? "active-content" : ""
                  }`}
                >
                  <div className="peer-section">
                    <Table
                      columns={peerColumn}
                      dataSource={user?.peers}
                      pagination={false}
                    />
                  </div>
                </div>
                ...
                <div
                  className={`content ${
                    activeTab === 5 ? "active-content" : ""
                  }`}
                >
                  <div className="peer-section">
                    <Table
                      columns={peerColumn}
                      dataSource={user?.peerRequest}
                      pagination={false}
                    />
                  </div>
                </div>
